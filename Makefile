...
build:
		docker build -t mutation_testing_example .

erase: ## Remove the Docker containers of the project
		docker rmi mutation_testing_example

composer-install: ## Install composer dependencies
		docker run --rm -v .:/app mutation_testing_example composer install

composer-update: ## Update composer dependencies
		docker run --rm -v .:/app mutation_testing_example composer update

phpunit: ## Run PHPUnit
		docker run --rm -v .:/app mutation_testing_example vendor/bin/phpunit tests

infection: ## Run PHPUnit
		docker run --rm -v .:/app mutation_testing_example vendor/bin/infection
...
.PHONY: tests
tests: ## Run cs validation and PHPUnit
	make ci-tests
	make phpunit

ci-tests: ## Run pipeline tests
	make validate-cs
